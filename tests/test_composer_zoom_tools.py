...
def test_zoom_drags_up_to_zoom_in_about_the_press_point(composer):
    ...
    # anchored: within the scroll bars' integer rounding — up to two
    # pixels on each axis at the zoom reached (a fixed 0.5 mm failed on
    # the CI runner's smaller screen, where a pixel is more paper)
    slack = 2.0 / view.transform().m11()
    d = view.mapToScene(press) - under
    assert abs(d.x()) < slack and abs(d.y()) < slack
