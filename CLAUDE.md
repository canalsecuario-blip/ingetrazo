...
## 📦 Estado (2026-09-14) — norte del proyecto: el mapa gira bajo el modelo

**Noche 14-09 (tras la 0.3.20, sin publicar) — revisión de Rafael B1–B4 e inferencia a la altura de SketchUp.** Planos de rectángulo/círculo/arcos siguen la vista (`PlaneLock.hover_plane`, `6266a05`) y muestran el plano en el cursor (anillo/cuadrado, `d813148`); «Desde el punto» antes del primer clic con puntos animados por PAUSA de 250 ms (`Viewport.ENCOURAGE_MS`, dos puntos → cruce), tangente en el vértice, punto medio de arco, origen de componente, «Sobre la línea» (`73dcce7`); todo alcanza grupos/componentes anidados (`pick_edge_any`, `_component_origin_points`, `_arc_midpoints`, `98630b2`); Empujar/Tirar infiere «En arista» (`acb1272`). **Planos de caras de componentes ANIDADOS:** `pick_face_any` devuelve el DUEÑO (selección); todo plano se lee por `pick_face_placement` (colocación con matriz compuesta; `idx.ent_place_idx`, `b14bf27`); el bloque del índice de picking se firma por dueño (`4ad4ffb`). **Esc:** una sola cascada `Viewport.escape()` (la QAction «Cancelar herramienta» con atajo Esc se disparaba antes que el visor y no salía del grupo, `04f5347`). **Rendimiento medido** (`scripts/bench_session.py`, nuevo `scripts/bench_nested.py`): oclusión perezosa de candidatos (`f73bd07`, hover con Línea ×2), barra de estado a 12 Hz (`98e9e1b`), GC `set_threshold(50000)` — `gc.freeze` medido y descartado (`d5291da`), caja del contenedor con prefiltro de 16 direcciones + caché de arrays por prototipo validada con `Mesh._mut_serial` (entrar en la plaza 473 → 126 ms). Método que funcionó: sondas por el puente (`_probe`), vídeo por fotogramas con ffmpeg. Wayland con escala fraccionaria pinta peor que xcb (p90 149 vs 30 ms): pendiente decidir el arranque.

**v0.3.20 released 2026-09-14 (noche)** (`85fd917`, tag `v0.3.20`; suite 2853 verdes). Cierra la sesión de la tarde: ortomosaico GeoTIFF, escenas↔mapa base/sombras, etiquetas multiguía con fondo ajustado, plumas ráster, Preferencia de tamaño de icono (`views.icons.default_toolbar_icon_px`: 32 px si la pantalla disponible ≥ 900 px de alto, 24 si no — medido: a 32 px la fábrica desborda 1366×768), ~25 iconos redibujados a elección de Marco, distribución de fábrica `resources/ui/default_layout.state`, **barra lateral con manija en la línea de redimensión** (`_build_sidebar_handle`/`_place_sidebar_handle` en las dos ventanas; se re-sube al cambiar de pestaña de dock), **Ctrl+0** con botón de salida en modelo y compositor, listas de bandeja sin scroll anidado (`views.tray.fit_rows`), compositor con barras movibles guardadas en `composer/window_state` y fábrica Lámina → **Dibujo** (barra nueva: 23 iconos en columna no cabían en laptop) → Organizar (VISIBLE por defecto desde hoy, revierte el 05-09), botón de desbordamiento con doble flecha (`style_overflow_button`). Rafael parte A (`cb9e86f`): capturas en metainfo, `setDesktopFileName(FLATPAK_ID)`, `core/appimage.py`; A2 («Abrir» de GNOME Software) cerrada solo con el desktop-id, no reproducida. Gotcha del puente: nunca reemplazar en caliente `eventFilter`/`closeEvent`/`paintGL` (virtuales) — para la manija se usó un QObject filtro auxiliar. Pendientes de Marco: repo Flatpak a mano, skp2dae.exe, web+manual. Pendiente: DWG como grupo único + «bajar al plano de referencia»; decidir si Anotar pasa arriba en la fábrica del modelo.

**Hecho 14-09** (sesión Plaza Yanque, la plaza estaba girada 33,5° como grupo para encajar en el satélite y las vistas Frontal/Derecha dejaron de servir): `SceneDatum.north` (grados HORARIOS desde el eje verde, misma convención que Solar North de SketchUp) gira mapa base, terreno, importaciones geográficas y el sol (`Viewport._sun_dir_now`) bajo el modelo; campo **Norte** y botón **Enderezar modelo sobre el mapa** en el panel Terreno/Mapa base; `StraightenModelCommand` (core/history.py) aplica la inversa de la colocación del grupo a TODO (grupos, malla suelta, cotas, textos, guías, planos de sección, imágenes, rutas/puntos, escenas, marcos y cotas ancladas de láminas), deja el grupo en identidad y reancla el datum (origen = esquina del grupo, norte += giro). Tests en `tests/test_north_angle.py`; manual `terreno/ubicacion.md`. Persistencia `.igz`: clave `north` solo si ≠ 0 (lectores viejos la ignoran).

**Hecho 14-09 (tarde, dogfooding de la Lámina 1 de Yanque con el DWG topográfico y el ortomosaico):** (1) **las escenas recuerdan mapa base / terreno / levantamiento (`SavedView.georef`) y las sombras (`SavedView.shadows`)** y el compositor aplica y devuelve ambos por marco (`_with_frame_camera`); escenas viejas = sin dato = no tocan (hay que «Actualizar escena»). (2) **Opacidad del mapa base** (`TileLayer.opacity`, deslizador en el panel, `u_opacity` en `_render_tiles`, `.igz`). (3) **Plumas ráster**: los marcos ráster toman las plumas Aristas/Perfiles — el render redibuja sus líneas GL con desplazamientos subpíxel (`Viewport._line_jitter`, `_export_edge_px/_export_profile_px`, `core.composition.pen_px`); 1 px a 300 dpi era un pelo invisible. El híbrido estilo LayOut queda DESCARTADO por ahora: el HLR exacto tarda 193 s en la plaza. (4) **Bug de sombras + imagen translúcida**: `_draw_image_planes` apagaba el blending y el receptor de sombras escribía alfa 0 (agujero blanco) en los frames que reutilizaban el mapa de sombras; ya no toca el blending; test con GL real `tests/test_image_plane_blend_state.py`. (5) **Importar ortomosaico GeoTIFF** (`georef/geotiff.py`, puro Python+NumPy; JPEG por Qt — Qt devuelve el JPEG de 4 canales como CMYK8888 INVERTIDO; LZW en pool de procesos; reduce en streaming a ≤8192 px) → `ImagePlane` bloqueado sobre el datum, WebP en la caché; `Archivo ▸ Importar ▸ Ortomosaico`; medido 0,1 s / 6,8 s (170 MP Deflate) / 33 s (665 MP LZW); «Opacidad de la imagen…» y el clic derecho alcanza imágenes bloqueadas (Desbloquear / Eliminar) aunque haya geometría encima. (6) Compositor: la pestaña Diseño ya no salta a propiedades en cada spinbox (`_panel_model`); etiquetas con guía: fondo que abraza las letras (`text_bg_span_mm`, medido contra el pintor: 0,8 mm sobre mayúsculas, 0,3 bajo descendentes), asa de ancho funcional (y dentro de `shape()`), editor en sitio alineado (sin margen de documento), **multiguía** (`EtiquetaItem.leaders`: clic del tool sobre una etiqueta existente, Ctrl-arrastre de una punta, soltar sobre las palabras la quita), punto al inicio de la guía (`dot`). (7) Enderezar modelo avisa si el grupo elegido no está girado (`placement_is_identity`) — Marco lo pulsó 11 veces sobre el DWG en vez de la plaza. **Gotcha de método (puente IA):** reemplazar en caliente un override virtual de Qt (`paintGL`, `boundingRect`, `paint`, `mouse*Event`) puede dejar a Shiboken llamando al slot liberado del objeto viejo — sale «fix_missing_locations._fix() missing 4 args» y el visor/ítem deja de pintarse; en caliente solo funciones de módulo y métodos Python normales, los virtuales → relanzar. **Pendientes que salieron:** importar el DWG como UN grupo con las capas dentro (Marco movió solo «limite» y las otras 8 capas quedaron 3 km arriba en Z) y ofrecer «bajar al plano de referencia» cuando el dibujo trae cotas.

**📋 PRÓXIMA SESIÓN (pedido de Marco, 14-09): implementar EJES estilo SketchUp** — (1) el gesto: **fijar el norte con dos clics** (punto + dirección, vista previa girando el mapa en vivo) y **reubicar el origen** con un clic sobre un punto del modelo (reusar la maquinaria de `StraightenModelCommand`: transformación rígida de todo + reanclar el datum); (2) la **herramienta Ejes** propiamente (ejes de dibujo con origen y orientación propios: inferencias rojo/verde/azul, bloqueos con flechas, rectángulo, vistas estándar y dibujo de los ejes siguen a los ejes nuevos; menú Colocar / Mover / Restablecer / Alinear vista) — útil para piezas giradas DENTRO de la plaza. Pendiente menor: que la flecha de norte de las láminas pueda tomar el norte del proyecto.
